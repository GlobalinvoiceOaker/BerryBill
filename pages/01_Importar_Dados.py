...
with col2:
    st.markdown('<div class="main-header">Importar Dados</div>',
                unsafe_allow_html=True)
    st.markdown(
        '<div class="description">Faça upload ou insira manualmente dados de venda dos parceiros (Masters)</div>',
        unsafe_allow_html=True)
...
with tab1:
    ...
    uploaded_file = st.file_uploader(
        "Faça upload do arquivo Excel com dados de venda",
        type=["xlsx", "xls"])
...
with tab2:
    ...
    with st.form(key="manual_data_form"):
        # Data da transação
        transaction_date = st.date_input("Data da Transação",
                                         value=datetime.now())
    ...
    if submit_button:
        ...
        if 'manual_records' not in st.session_state:
            st.session_state.manual_records = pd.DataFrame(
                columns=["Date", "Partner", "Country", "Amount", "Currency"])

        # Adiciona o novo registro
        st.session_state.manual_records = pd.concat(
            [st.session_state.manual_records, manual_data], ignore_index=True)

        st.success(
            f"Registro adicionado: {partner_name} - {amount} {currency}")
    ...
    if 'manual_records' in st.session_state and not st.session_state.manual_records.empty:
        ...
        if st.button("Processar Dados Manuais"):
            with st.spinner("Processando dados..."):
                ...
                st.markdown('<div class="sub-header">Dados Processados</div>',
                            unsafe_allow_html=True)
                st.dataframe(processed_data, use_container_width=True)

                # Mostra o resumo
                st.markdown('<div class="sub-header">Resumo dos Dados</div>',
                            unsafe_allow_html=True)
                ...
                with col2:
                    st.metric("Valor Total",
                              f"R$ {processed_data['Amount'].sum():,.2f}")

                with col3:
                    st.metric("Total de Parceiros",
                              processed_data['Partner'].nunique())
                ...
                # Próximos passos
                st.success(
                    "Processamento de dados concluído! Agora você pode prosseguir para gerar faturas."
                )
...
if uploaded_file is not None:
    try:
        ...
        st.markdown(
            '<div class="sub-header">Visualização dos Dados Brutos</div>',
            unsafe_allow_html=True)
        ...
        if is_valid:
            st.success(
                "Validação de dados bem-sucedida! O arquivo enviado possui todas as colunas necessárias."
            )
            ...
            if st.button("Processar Dados"):
                with st.spinner("Processando dados..."):
                    ...
                    # Exibe os dados processados
                    st.markdown(
                        '<div class="sub-header">Dados Processados</div>',
                        unsafe_allow_html=True)
                    st.dataframe(processed_data, use_container_width=True)

                    # Mostra o resumo
                    st.markdown(
                        '<div class="sub-header">Resumo dos Dados</div>',
                        unsafe_allow_html=True)
                    ...
                    with st.expander("Ver Resumo Detalhado"):
                        ...
                        partner_summary = processed_data.groupby(
                            'Partner').agg({
                                'Amount': 'sum',
                                'Royalty Amount': 'sum',
                                'Ad Fund Amount': 'sum',
                        ...
                        country_summary = processed_data.groupby(
                            'Country').agg({
                                'Amount': 'sum',
                                'Royalty Amount': 'sum',
                                'Ad Fund Amount': 'sum',
        ...
        st.info(
            "Por favor, certifique-se de que está enviando um arquivo Excel válido (.xlsx ou .xls)"
        )
else:
    # Exibe o formato de amostra de dados
    st.info(
        "Por favor, faça upload de um arquivo Excel com dados de venda para prosseguir."
    )
